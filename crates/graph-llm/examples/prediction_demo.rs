...
const TRUE_NEGATIVES: &[(u64, u64)] = &[(1, 9), (2, 14), (3, 15), (7, 22), (4, 23)];
...
#[cfg(feature = "ollama")]
fn is_hi(node: u64) -> bool {
    MR_HI.contains(&node)
}
#[cfg(feature = "ollama")]
fn is_officer(node: u64) -> bool {
    OFFICER.contains(&node)
}
...
fn task1_faction_split(client: &graph_llm::OllamaClient) {
    ...
    let hi_entity = entities
        .iter()
        .find(|e| e.current.members.contains(&LocusId(0)));
    let off_entity = entities
        .iter()
        .find(|e| e.current.members.contains(&LocusId(33)));

    let (hi_correct, hi_total) = hi_entity
        .map(|e| {
    ...
    let (off_correct, off_total) = off_entity
        .map(|e| {
    ...
    println!(
        "    Mr. Hi  entity    : {hi_correct}/{hi_total} nodes correct ({:.0}%)",
        hi_correct as f32 / hi_total.max(1) as f32 * 100.0
    );
    println!(
        "    Officer entity    : {off_correct}/{off_total} nodes correct ({:.0}%)",
    ...
    let ground_truth = "Two factions split the club: Mr. Hi (node 0) led 16 members \
    ...
    let metrics = format!(
        ...
        MR_HI.len(),
        hi_correct as f32 / MR_HI.len() as f32 * 100.0,
        OFFICER.len(),
        off_correct as f32 / OFFICER.len() as f32 * 100.0,
    );

    println!("\n  LLM evaluation:");
    match score_prediction(
        client,
...
fn task2_link_prediction(client: &graph_llm::OllamaClient) {
    use graph_core::{
        ...
        LocusKindId, LocusProgram, ProposedChange, Relationship, RelationshipLineage, StateVector,
    };
    use graph_engine::{
        DefaultEmergencePerspective, Engine, InfluenceKindConfig, InfluenceKindRegistry,
        LocusKindRegistry,
    };
    use graph_llm::score_prediction;
    use graph_world::World;
    ...
    let training_edges: Vec<(u64, u64)> = ALL_EDGES
        .iter()
        .copied()
    ...
    let holdout_hits: usize = HOLDOUT_EDGES
        .iter()
        .filter(|&&(a, b)| same_entity(a, b))
        .count();
    let tn_hits: usize = TRUE_NEGATIVES
        .iter()
        .filter(|&&(a, b)| same_entity(a, b))
        .count();
    ...
    println!("  Results:");
    println!(
        "    Held-out edges predicted  : {holdout_hits}/{} ({precision:.0}% recall)",
        HOLDOUT_EDGES.len()
    );
    println!(
        "    True negatives rejected   : {}/{} ({tn_acc:.0}% specificity)",
        TRUE_NEGATIVES.len() - tn_hits,
        TRUE_NEGATIVES.len()
    );

    let holdout_detail: Vec<String> = HOLDOUT_EDGES
        .iter()
        .map(|&(a, b)| {
            format!(
                "  ({a},{b}) → {}",
        ...
        .collect();
    let tn_detail: Vec<String> = TRUE_NEGATIVES
        .iter()
        .map(|&(a, b)| {
            format!(
                "  ({a},{b}) → {}",
    ...
    println!("\n  Held-out edges:");
    for line in &holdout_detail {
        println!("   {line}");
    }
    println!("\n  True negatives:");
    for line in &tn_detail {
        println!("   {line}");
    }
    ...
    let ground_truth = "All 10 held-out edges connect two nodes within the same faction. \
...
fn task3_trend_forecasting(client: &graph_llm::OllamaClient) {
    ...
    use graph_llm::score_prediction;
    use graph_query::{Trend, relationship_weight_trend_delta};
    use graph_world::World;

    println!("━━━ Task 3: Weight trend forecasting ━━━");
    println!(
        "  Setup  : 78 edges, 200 ticks of BCM plasticity (η=0.5 τ=3, alternating stimulation)"
    );
    println!(
        "  Method : relationship_weight_trend_delta (ChangeLog first-vs-last on slot 1, threshold=0.003)"
    );
    ...
    for tick in 0..200u64 {
        let leader = if tick % 2 == 0 {
            LocusId(0)
        } else {
            LocusId(33)
        };
        let stimuli = vec![ProposedChange::new(
    ...
    for &(a, b) in ALL_EDGES {
        let Some(&rel_id) = edge_to_rel.get(&(a, b)) else {
            continue;
        };
        let trend = relationship_weight_trend_delta(&world, rel_id, start_batch, end_batch, 0.003)
            .unwrap_or(Trend::Stable);
        let ti = trend_idx(&trend);
        if is_hi(a) && is_hi(b) {
            intra_hi_trends[ti] += 1;
        } else if is_officer(a) && is_officer(b) {
            intra_off_trends[ti] += 1;
        } else {
            cross_trends[ti] += 1;
        }
    }
    ...
    println!(
        "  Results (200 ticks alternating BCM η=0.5 τ=3, threshold=0.003, ChangeLog first-vs-last):"
    );
    println!("  ┌──────────────────┬─────────┬─────────┬──────────┐");
    println!("  │ Edge type        │ Rising  │ Stable  │ Falling  │");
    println!("  ├──────────────────┼─────────┼─────────┼──────────┤");
    println!(
        "  │ Intra-Hi ({:2})   │ {:3.0}%    │ {:3.0}%    │ {:3.0}%     │",
        intra_hi_total,
    ...
    let ground_truth = "Hebbian plasticity reinforces intra-faction edges (co-activated by the same leader \
