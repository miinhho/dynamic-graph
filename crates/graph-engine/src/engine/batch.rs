...
use graph_core::{
    ChangeId, Endpoints, ProposedChange, Relationship, RelationshipLineage,
    StateVector, StructuralProposal,
};
use graph_world::World;

/// Decay rates for a relationship's activity and weight slots, looked up
/// once per influence kind and passed to `auto_emerge_relationship`.
pub(crate) struct DecayParams {
    ...
    if let Some(rel_id) = store.lookup(&key, kind) {
        ...
        if delta > 0 {
            ...
            if let Some(a) = slots.get_mut(Relationship::ACTIVITY_SLOT) {
                *a *= decay.activity.powi(delta as i32);
            }
            if let Some(w) = slots.get_mut(Relationship::WEIGHT_SLOT) {
                *w *= decay.weight.powi(delta as i32);
            }
        }
...
pub(crate) fn apply_structural_proposals(world: &mut World, proposals: Vec<StructuralProposal>) {
