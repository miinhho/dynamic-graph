...
use graph_core::{
    Change, ChangeSubject, Endpoints, InfluenceKindId, Locus, LocusContext, LocusId, LocusProgram,
    ProposedChange, RelationshipId, RelationshipSlotDef, StateVector, StructuralProposal,
    changes_of_kind, relationship_changes_of_kind,
};
use graph_engine::{DefaultCoherePerspective, DefaultEmergencePerspective, SimulationBuilder};
...
impl LocusProgram for SupplierProgram {
    ...
    fn structural_proposals(
        ...
        println!(
            "    [SUPPLIER L{}] failure signal received — proposing DeleteLocus",
            locus.id.0
        );
...
impl LocusProgram for AnalystProgram {
    fn process(
        ...
        for change in relationship_changes_of_kind(incoming, SUPPLY_KIND) {
            ...
            let reliability = change
                .after
                .as_slice()
                .get(RELIABILITY_SLOT)
                .copied()
                .unwrap_or(0.0);
...
fn build_simulation() -> Setup {
    ...
    let mut builder = SimulationBuilder::new()
        ...
        .engine(|_| graph_engine::EngineConfig {
    ...
    builder
        .world_mut()
        .subscriptions_mut()
        .subscribe_at(ANALYST, sup_a_rel, None);
    builder
        .world_mut()
        .subscriptions_mut()
        .subscribe_at(ANALYST, sup_b_rel, None);
    ...
    let analyst_kind = builder.add_locus_kind(
        "ANALYST",
        AnalystProgram {
            sup_a_rel,
            sup_b_rel,
    ...
    builder
        .world_mut()
        .insert_locus(Locus::new(ANALYST, analyst_kind, StateVector::zeros(1)));
...
fn main() {
    ...
    println!(
        ...
        r1.tick.batches_committed,
        r1.tick.changes_committed,
        sim.world().relationships().len()
    );
    let stock = sim
        .world()
        .locus(WAREHOUSE)
        .map(|l| l.state.as_slice()[0])
        .unwrap_or(0.0);
    println!("  warehouse stock: {stock:.3}");
    println!(
        "  analyst subscriptions active: {}",
        sim.world().subscriptions().subscription_count()
    );
    ...
    println!(
        "  batches={} changes={} relationships={}",
        r2.tick.batches_committed,
        r2.tick.changes_committed,
        sim.world().relationships().len()
    );
    let stock = sim
        .world()
    ...
    println!(
        "  SUPPLIER_B exists: {}",
        sim.world().locus(SUPPLIER_B).is_some()
    );
    println!(
        "  analyst subscriptions active: {} (SUPPLIER_B edge cleaned up)",
        sim.world().subscriptions().subscription_count()
    );
    print_relationships(&sim);
    println!();
    ...
    let r3 = sim.step(vec![ProposedChange::stimulus(
        SUPPLIER_A,
        ORDER_KIND,
        &[1.0],
    )]);
    println!(
        "  batches={} changes={} relationships={}",
        r3.tick.batches_committed,
        r3.tick.changes_committed,
        sim.world().relationships().len()
    );
    let stock = sim
        .world()
...

    {
        let wg = sim.world();
        println!(
            "--- Change log summary ({} changes total) ---",
            wg.log().len()
        );
        ...
        println!(
            "  relationship-subject changes: {rel_changes}  (analyst received these via subscription)"
        );
        println!();
    ...
    match Q::relationship_activity_trend(&*sim.world(), sup_a_rel, from_batch, to_batch) {
        ...
        Some(Q::Trend::Stable) => println!("  trend=Stable  (activity steady across batches)"),
        None => {
