...
pub(super) fn psi_scalar_inner(
    ...
    for relationship_id in &entity.current.member_relationships {
        let component_series: Vec<f64> = prepared
            .window
            .iter()
        ...
        if let Some(mi) = gaussian_mi_from_series(&component_series, &prepared.v_t1) {
            component_sum += mi;
            n_components += 1;
        }
    }
...
pub(super) fn psi_synergy_inner(
    ...
    if prepared.n_pairs < n_components + 2 {
        return None;
    }
